...
The package is ESM-only (`"type": "module"`). There are no build or test scripts defined.

## Architecture

The library exports three main factory functions that follow a plugin composition pattern:
...
- `client-request` / `client-response` — request/response with a generated UID to match responses to waiting promises
- `service-event` — server-to-client pub/sub notifications
- `app-event` — application-wide broadcast events (outside any service)

The `service(name, options)` method returns a `Proxy` that intercepts any property access and turns it into a `serviceMethodRequest` call, so callers can write `app.service('user').findMany(...)` without pre-declaring methods.
...
- `Mutex` — simple async mutex backed by a promise queue
- `wherePredicate(where)` — turns a `where` object into a filter function
- `isSubset` / `isSubsetAmong` — checks if a `where` is covered by an existing entry in `whereList` (avoids redundant syncs)
- `stringifyWithSortedKeys` — deterministic JSON stringify used as canonical `whereList` keys
- `generateUID(length)` — alphanumeric random string used to correlate request/response pairs
